...
- NO approximations. Return comprehensive data."""
    ...
    for attempt in range(max_retries):
        try:
            response = requests.post("[https://api.perplexity.ai/chat/completions](https://api.perplexity.ai/chat/completions)", 
...
with tab2:
    ...
    if not st.session_state.main_topic: st.warning("Set topic in Tab 1"); st.stop()
    
    if st.button("Generate Research Queries"):
        res, err = generate_research_queries(st.session_state.main_topic)
        if res: st.session_state.fanout_results = res; st.rerun()
    
    if st.session_state.fanout_results:
        queries = st.session_state.fanout_results['queries']
        st.write(f"Generated {len(queries)} queries. Select to research:")
        
        # Selection Logic
        for i, q in enumerate(queries):
            qid = f"q_{i}"
            if st.checkbox(q['query'], key=qid): st.session_state.selected_queries.add(qid)
            else: st.session_state.selected_queries.discard(qid)
            
        if st.button("Run Research on Selected"):
            with st.spinner("Researching..."):
                for qid in st.session_state.selected_queries:
                    idx = int(qid.split('_')[1])
                    q_text = queries[idx]['query']
                    res = call_perplexity(q_text)
                    if 'choices' in res:
                        st.session_state.research_results[qid] = {'query': q_text, 'result': res['choices'][0]['message']['content']}
